...
try {
    $health = Invoke-RestMethod -Uri 'http://localhost:8080/health-test'
...
try {
    $adminLogin = @{username='admin'; password='admin'} | ConvertTo-Json
...
try {
    $userLogin = @{username='user'; password='user'} | ConvertTo-Json
    $userResponse = Invoke-RestMethod -Uri 'http://localhost:8080/api/auth/login' -Method POST -Body $userLogin -ContentType 'application/json'
...
try {
    ...
    $profile = Invoke-RestMethod -Uri 'http://localhost:8080/api/secure/profile' -Headers $headers
...
try {
    $headers = @{Authorization = "Bearer $adminToken"}
    $admin = Invoke-RestMethod -Uri 'http://localhost:8080/api/secure/admin' -Headers $headers
...
try {
    $headers = @{Authorization = "Bearer $adminToken"}
    $user = Invoke-RestMethod -Uri 'http://localhost:8080/api/secure/user' -Headers $headers
...
try {
    $headers = @{Authorization = "Bearer $userToken"}
    Invoke-RestMethod -Uri 'http://localhost:8080/api/secure/admin' -Headers $headers
...
try {
    Invoke-RestMethod -Uri 'http://localhost:8080/api/secure/profile'
    Write-Host "   ❌ FALHOU - deveria bloquear!" -ForegroundColor Red
} catch {
...
try {
    ...
    Invoke-RestMethod -Uri 'http://localhost:8080/api/secure/profile' -Headers $headers
    Write-Host "   ❌ FALHOU - deveria bloquear!" -ForegroundColor Red
} catch {
    if ($_.Exception.Response.StatusCode -eq 'Unauthorized') {
        Write-Host "   ✅ Bloqueado com 401 Unauthorized (esperado)" -ForegroundColor Green
